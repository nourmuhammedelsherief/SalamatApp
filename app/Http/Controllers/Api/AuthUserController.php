...
//        $user = User::wherePhone($request->phone)->first();
    ...
    {
        $reasons = App\Reson::select('id', 'reason')->get();
        $data = [];
        array_push($data, ['reason' => $reasons]);
    ...
    public function delete_order(Request $request, $order_id)
    {
        ...
        $reason = App\Reson::where('id', $request->reason_id)->first();
        if ($reason) {
            $created = App\UserReason::create([
            ...
            array_push($all, [
                ...
                'user_reason' => $created->user_reason,
                'user_id' => intval($request->user()->id)
            ]);
//            $orders = App\salamatOfferPrice::where('order_id', $order_id)
//                ->get();
            if ($order->price  == null) {
                $order = App\Order::find($order_id)->update(['status' => 4 ,'cancel' => 1]);
                $errors = ['key' => 'message',
                    'value' => "تم الغاء طلبك بنجاح"
                ];
                return ApiController::respondWithSuccess(array($errors));
            } else {
                $orders = App\Order::find($order_id)->update(['cancel' => 1]);
                $errors = ['key' => 'message',
                    'value' => "سوف يتم مراجعته من قبل الاداره قبل الغاء الطلب"
                ];
                return ApiController::respondWithSuccess(array($errors));
